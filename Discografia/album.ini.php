<?php
    class Album{
        ...
        public function registrarDisco($conexion){
            try{
            $consulta = $conexion->exec('INSERT INTO discografia.album (titulo,discografica,formato,fechaLanzamiento,fechaCompra,precio) VALUES ("'. $this->getTitulo() .'","'. $this->getDiscografia() .'","'.$this->getFormato().'","'. $this->getFechaL() .'","'.$this->getFechaC().'",'.$this->precio.');');
        ...
        function borrarDisco($conexion,$tc){
            try{
                ...
                if($tc != 0){
                    ...
                    $consulta2 = 'DELETE from discografia.album where codigo = '.$this->cod.';';
                    ...
                    $consulta = $conexion->exec('DELETE from discografia.album where codigo = '.$this->cod.';');
                    echo'<h1 id="bien">SE BORRÓ EL DISCO CON ÉXITO!</h1>';
                }
            }catch(Exception $e){
                echo '<h1 id="mal">ERROR AL BORRAR EL DISCO!</h1>';
