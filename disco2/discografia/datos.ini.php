<?php
	function formularioDisco(){//Esta función imprimirá en la página un formulario y llamará a la función registrar del objeto para registrar un disco nuevo
		...
		echo '<input type="text" required name="discografica" placeholder="Discografía"/>';
		...
		if(isset($_POST["titulo"])){
			...
			$album = new Album('',$_POST['titulo'],$_POST['discografica'],$_POST['formato'],$_POST['fechaLanzamiento'],$_POST['fechaCompra'],$_POST['precio']);
	...
	function datosDiscografia(){//devuelve una lista de Albums
		...
		while ($registro = $resultado->fetch()) {
			...
			echo '<tr>';
			echo '<td><a href="http://localhost/disco2/discografia/disco.php?codigo='.$album->getCod().'">'.$album->getTitulo().'</a></td>';
			...
			echo '<th id="botonInsertar" ><button  onclick=location.href="./cancionnueva.php?codigo='.$registro['codigo'].'&titulo='.$registro['titulo'].'">Canción Nueva</button></th>';
			echo '</tr>';
		}
		echo'</table>';
	}
	...
	function datosDisco($album){//Devuelve los datos del album seleccionado
		...
		while ($registro = $resultado->fetch()) {
			...
			echo '<th id="botonBorrar" ><button  onclick=location.href="./borrardisco.php?codigo='.$listaAlbum->getCod().'&TC='.$TC.'">Borrar disco</button></th>';
			echo '</tr>';
		}
		echo'</table>';
	...
	function datosCancion($codigo){//devuelve los datos de todas las canciones del album pasado
		$conectar = new Conexion('localhost','discografia','discografia','discografia');
		$conexion = $conectar->conectionPDO();
		$resultado = $conexion->query('SELECT * FROM discografia.cancion WHERE album = '.$codigo.';');
	...
	function datosBuscados($textoBuscar, $select, $genero){//Esta función devuelve una lista de canciones dependiendo de los datos que quiera utilizar el usuario para su busqueda
		$conectar = new Conexion('localhost','discografia','discografia','discografia');
		$conexion = $conectar->conectionPDO();
		$resultado1 = $conexion->query('SELECT count(cancion.titulo) as cont FROM discografia.cancion,discografia.album WHERE album.codigo = cancion.album and cancion.genero = "'.$genero.'" and '.$select.' LIKE "%'.$textoBuscar.'%";');
		
		$contar = $resultado1->fetch();
		if($contar['cont'] > 0) {
			$resultado2 = $conexion->query('SELECT cancion.titulo as titulo ,album.titulo as album, cancion.posicion, cancion.duracion, cancion.genero FROM discografia.cancion,discografia.album WHERE album.codigo = cancion.album and cancion.genero = "'.$genero.'" and '.$select.' LIKE "%'.$textoBuscar.'%";');
		echo'<table>';
		echo'<tr>
			<th>título</th>
			<th>Album</th>
